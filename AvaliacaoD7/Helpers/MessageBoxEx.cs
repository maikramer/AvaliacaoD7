...
public class MessageBoxEx
{
    ...
    [DllImport("user32.dll")]
    private static extern int MoveWindow(IntPtr hWnd, int X, int Y, int nWidth, int nHeight,
        bool bRepaint);
    ...
    [DllImport("user32.dll")]
    public static extern IntPtr SetWindowsHookEx(int idHook, HookProc lpfn, IntPtr hInstance,
        int threadId);
    ...
    [DllImport("user32.dll")]
    public static extern IntPtr CallNextHookEx(IntPtr idHook, int nCode, IntPtr wParam,
        IntPtr lParam);
    ...
    private static void CenterWindow(IntPtr hChildWnd)
    {
        ...
        {
            X = (ptCenter.X - (width / 2)), Y = (ptCenter.Y - (height / 2))
        };

        ptStart.X = (ptStart.X < 0) ? 0 : ptStart.X;
        ptStart.Y = (ptStart.Y < 0) ? 0 : ptStart.Y;

        var unused = MoveWindow(hChildWnd, ptStart.X, ptStart.Y, width, height, false);
    }
}